resource "aws_vpc" "main" {
  ...
  #required fro eks to work
  #so worker nodes can resolve aws dns names like ecr endpoints
...
resource "aws_subnet" "public" {
  ...
  #anything launched here gets a public IP
...
resource "aws_subnet" "private" {
  ...
  # The internal-elb tag tells AWS these subnets are for internal load balancers only.
  tags = merge(local.tags, {
...
#  Gives private subnet resources outbound internet access
#  so worker nodes can pull images from ECR, call AWS APIs etc
